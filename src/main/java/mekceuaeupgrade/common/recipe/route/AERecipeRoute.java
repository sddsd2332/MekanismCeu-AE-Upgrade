 ...
 *
 * <p>route 保存 typed 输入和 typed 输出，并在暴露到 AE 时转换成现有 adapter 使用的输入载体表达。
...
public record AERecipeRoute(String routeId, List<AERecipeRouteStack> inputs, List<AERecipeRouteStack> outputs) {
    ...
    public AEExposedRecipe toLegacyRecipe() {
        List<ItemStack> carrierInputs = toCarrierStacks(inputs);
        List<ItemStack> carrierOutputs = toCarrierStacks(outputs);
        if (carrierInputs == null || carrierOutputs == null || carrierInputs.isEmpty() || carrierOutputs.isEmpty()) {
            return null;
        }
        return new AEExposedRecipe(carrierInputs, carrierOutputs, this, getRouteDiscriminator());
    }
    ...
    /**
     * 将 typed 栈列表转换成 AE 输入/输出载体列表。
     *
     * @param routeStacks typed route 栈列表
     * @return 全部可转换时返回载体栈列表，否则返回 null
     */
    @Nullable
    private static List<ItemStack> toCarrierStacks(List<AERecipeRouteStack> routeStacks) {
        List<ItemStack> stacks = new ArrayList<>(routeStacks.size());
        for (AERecipeRouteStack routeStack : routeStacks) {
            ItemStack stack = routeStack.toCarrierStack();
            if (stack.isEmpty()) {
                return null;
            }
            stacks.add(stack);
    ...
    private static void appendStackIdentity(StringBuilder builder, AERecipeRouteStack stack) {
        switch (stack.kind()) {
            ...
            case GAS -> {
                ...
                if (!stack.carrierItemStack().isEmpty()) {
                    appendItemIdentity(builder.append(":carrier="), stack.carrierItemStack());
                }
            }
            case FLUID -> {
    ...
    public static final class Builder {
        ...
        /**
         * 追加气体输入，并指定 AE pattern 中使用的兼容物品代表。
         *
         * @param portId 绑定机器输入端口的 ID
         * @param stack 真实气体输入
         * @param carrierItemStack AE pattern 中代表该气体输入的兼容物品
         * @return 当前构建器
         */
        public Builder inputGas(String portId, GasStack stack, ItemStack carrierItemStack) {
            inputs.add(AERecipeRouteStack.gas(portId, stack, carrierItemStack));
            return this;
        }

...
/**
 * route typed 栈与 AE 交付输入之间的转换和匹配工具。
 *
 * <p>现有机器 adapter 仍以 {@link ItemStack} 作为输入载体；该类负责判断载体是否符合 route 类型，
 * 并把 Supergiant generic wrapper、气体转换物品解码回真实 Mekanism 栈。</p>
 */
public final class AERecipeRouteLegacyIO {
    ...
    /**
     * 检查 AE 输入载体列表是否匹配暴露配方及其 route 类型。
     *
    ...
    /**
     * 检查单个 AE 输入载体是否匹配暴露配方及其 route 类型。
     *
     * @param recipe 暴露给 AE 的配方
    ...
    /**
     * 检查单个 AE 输入载体是否匹配指定 route 栈。
     *
     * @param routeStack route 中的 typed 输入栈
     * @param supplied AE 实际交付的输入载体
     ...
     * 从 AE 输入载体中解析气体来源。
     *
     * <p>优先解析 Supergiant generic gas wrapper；如果不是气体容器，则尝试 Mekanism 的物品转气体表。</p>
     *
     * @param stack AE 实际交付的输入载体
    ...
    /**
     * 检查单个 AE 输入载体是否能作为 route 栈对应的状态。
     *
     * @param routeStack route 中的 typed 输入栈
     * @param supplied AE 实际交付的输入载体
     * @return 状态匹配时返回 true

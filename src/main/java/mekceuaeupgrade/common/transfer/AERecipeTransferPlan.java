...
public final class AERecipeTransferPlan {
    ...
    /**
     * 按输入顺序从 AE 旧物品输入创建转移计划。
     *
     * <p>没有 route 的旧 item-only 配方会按物品输入处理；带 route 的配方会根据 route 的类型把假气体或假流体物品解码为真实栈。</p>
     *
     * @param recipe 暴露给 AE 的配方
    ...
    /**
     * 按 route 端口 ID 从 AE 旧物品输入创建转移计划。
     *
    ...
    /**
     * 将 AE 旧物品输入解码成端口可写入的 typed route 栈。
     *
     * @param recipe 暴露给 AE 的配方
    ...
    private static AERecipeRouteStack decodeInput(AEExposedRecipe recipe, @Nullable AERecipeRoute route, List<ItemStack> stacks, int index,
        ...
        ItemStack legacyStack = stacks.get(index);
        if (routeStack != null && !AERecipeRouteLegacyIO.matchesInput(routeStack, legacyStack)) {
            return null;
        }
        switch (kind) {
            case ITEM:
                return legacyStack == null || legacyStack.isEmpty() ? null : AERecipeRouteStack.item(port.portId(), legacyStack);
            case GAS:
                if (routeStack == null) {
                    return null;
                }

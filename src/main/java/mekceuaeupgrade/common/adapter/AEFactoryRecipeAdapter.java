...
public final class AEFactoryRecipeAdapter implements IAERecipeMachineAdapter {
    ...
    public boolean drainItemOutputs(IAEItemRecipeHost host, AEUpgradeNode node) {
        ...
        for (IAEFactoryRecipeHost.ProcessView processInfo : factory.getAEFactoryProcesses()) {
            drained |= AERecipePort.drainItems(node, processInfo.outputSlot(), processInfo.secondaryOutputSlot());
        }
        if (isAEPressurizedRecipeType(factory)) {
            drained |= AERecipePort.drainGas(node, factory.getAEFactoryGasOutputTank());
        }
        return drained;
    }
    ...
    private boolean canFactoryProcessProduce(IAEFactoryRecipeHost factory, FactoryBatch batch, IAEFactoryRecipeHost.ProcessView process) {
    ...
    @Nullable
    private IAEFactoryRecipeHost.ProcessView rejectAEItemInputProcess(IAEFactoryRecipeHost factory, String reason) {
        rejectAEItemInput(factory, reason);
        return null;
    }

    @Nullable
    private IAEFactoryRecipeHost.ProcessView rejectAEItemInputProcess(IAEFactoryRecipeHost factory, String reason, Object arg0) {
        rejectAEItemInput(factory, reason, arg0);
        return null;
    }

    @Nullable
    private IAEFactoryRecipeHost.ProcessView rejectAEItemInputProcess(IAEFactoryRecipeHost factory, String reason, Object arg0, Object arg1) {
        rejectAEItemInput(factory, reason, arg0, arg1);
        return null;
    }
    ...
    private boolean rejectAEItemInput(IAEFactoryRecipeHost factory, String reason) {
        if (AEUpgradeDebug.enabled()) {
            AEUpgradeDebug.log(factory, "rejecting AE item input: " + reason);
        }
        return false;
    }

    private boolean rejectAEItemInput(IAEFactoryRecipeHost factory, String reason, Object arg0) {
        if (AEUpgradeDebug.enabled()) {
            AEUpgradeDebug.log(factory, "rejecting AE item input: " + reason, arg0);
        }
        return false;
    }

    private boolean rejectAEItemInput(IAEFactoryRecipeHost factory, String reason, Object arg0, Object arg1) {
        if (AEUpgradeDebug.enabled()) {
            AEUpgradeDebug.log(factory, "rejecting AE item input: " + reason, arg0, arg1);
        }
        return false;
    }

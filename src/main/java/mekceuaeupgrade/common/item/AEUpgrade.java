    ...
    private static void handleAEUpgradeChanged(IAEUpgradeHost host, Upgrade upgrade, int previousAmount, int amount) {
        if (amount <= 0 && previousAmount > 0) {
            if (upgrade == AE_WIRELESS_CRAFTING) {
                host.getAEUpgradeNode().setWirelessCraftingLink(null);
            } else if (upgrade == AE_WIRELESS_AUTO_PROCESSING) {
                host.getAEUpgradeNode().setWirelessAutoProcessingLink(null);
            } else if (upgrade == AE_WIRELESS_OUTPUT) {
                host.getAEUpgradeNode().setWirelessOutputLink(null);
            }
        }
